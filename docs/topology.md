...
- `Layered` uses node kind or `Metadata["layer"]` for top-to-bottom or left-to-right layers. Crowded levels wrap inside their layer band; hierarchy levels with visible parents are bucketed near those parents before falling back to the generic grid. Multi-child hierarchy fan-outs receive shared connector bus waypoints and expose `hierarchy.route = shared-bus` on generated edges. Prepared nodes expose `layout.layer`, `layout.layerIndex`, `layout.row`, and `layout.column` metadata for host diagnostics.

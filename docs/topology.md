...
Artwork-backed icons add an optional `artwork` object to the icon manifest. Use `svg` for a trusted SVG fragment inside the supplied `svgViewBox`, or `imageHref` for a host-managed asset URL or data URI. The icon still needs `shape`, `symbol`, and `color` because those are the fallback language for unsupported artwork, text-only exports, and hosts that choose not to embed artwork. For diagram-specific artwork that should not become a reusable icon pack, add it directly with `.AddArtworkNode("edge-cloud", "Service Edge", TopologyIconArtwork.InlineSvg(...).WithPreserveAspectRatio("none"), x, y)` or `.AddArtworkNode("logo", "Logo", TopologyIconArtwork.Image(...), x, y)`, or apply the same `TopologyIconArtwork` instance to an existing node with `.WithNodeArtwork(...)`; all node-artwork helpers default the node to `TopologyNodeDisplayMode.Artwork` and scale safe SVG or image-href artwork to the full node bounds. PNG output rasterizes safe inline SVG artwork through ChartForgeX's dependency-free SVG raster layer, including common paths, arcs, rectangles, circles, ellipses, lines, polygons, polylines, `fill-rule`, `clip-rule`, `symbol` and `use` references, `clipPath`, `mask`, `pattern`, `linearGradient` and `radialGradient` fills from `defs`, referenced gradient and pattern definitions through `href`, `gradientTransform`, `patternTransform`, `spreadMethod`, nested SVG viewBoxes, presentation attributes, inline style declarations, embedded stylesheet rules for simple element/class/id selectors, same-element compound selectors, and descendant selectors, CSS custom properties with `var()` fallbacks, CSS `rgb()`/`rgba()` color functions, transforms, opacity, stroke caps, joins, dash arrays, `text`/`tspan` runs with basic anchor and baseline alignment, and text fill styling. `.WithoutNodeArtwork(...)` removes a direct override so a node can fall back to its catalog icon artwork and display defaults. If a node has both `IconId` and direct `Artwork`, safe direct artwork wins for SVG/HTML/PNG rendering while the icon id, pack, label, and shape metadata remain available for host inspectors and fallback language. Unsafe or non-embeddable direct artwork is not advertised in renderer metadata and can fall back to safe catalog artwork.

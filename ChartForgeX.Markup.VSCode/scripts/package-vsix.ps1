...
$cliProject = Join-Path $repoRoot 'ChartForgeX.Markup.Cli/ChartForgeX.Markup.Cli.csproj'
...
if (-not (Test-Path -LiteralPath $cliProject)) {
    throw "ChartForgeX.Markup.Cli project not found at $cliProject."
}
...
try {
    ...
    $bundledCli = Assert-ChildPath -Path (Join-Path $extensionRoot 'tools/ChartForgeX.Markup.Cli') -Parent $extensionRoot
    ...
    Write-Host "Publishing ChartForgeX.Markup.Cli ($Configuration, $Framework, portable fallback)..." -ForegroundColor Cyan
    ...
    foreach ($rid in $RuntimeIdentifiers) {
        ...
        Write-Host "Publishing ChartForgeX.Markup.Cli ($Configuration, $Framework, $rid, self-contained)..." -ForegroundColor Cyan

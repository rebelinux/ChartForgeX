...
function parseArgs(argv) {
  ...
  if (!options.runtimeIdentifiers.length) throw new Error('At least one runtime identifier must be provided.');
  return options;
}
...
function run(command, args, options = {}) {
  ...
  const argumentList = isWindowsScript ? ['/d', '/s', '/c', quoteWindowsCommand(command, args)] : args;
...
function vsceCommand(extensionRoot) {
  ...
  if (fs.existsSync(windowsVsce)) return { command: windowsVsce, prefix: [] };
  const posixVsce = path.join(extensionRoot, 'node_modules', '.bin', 'vsce');
  if (fs.existsSync(posixVsce)) return { command: posixVsce, prefix: [] };
  const vsceMain = path.join(extensionRoot, 'node_modules', '@vscode', 'vsce', 'out', 'main.js');
  if (fs.existsSync(vsceMain)) return { command: process.execPath, prefix: [vsceMain] };
...
const cliProject = path.join(repoRoot, 'ChartForgeX.Markup.Cli', 'ChartForgeX.Markup.Cli.csproj');

if (!fs.existsSync(packagePath)) throw new Error(`package.json not found at ${packagePath}.`);
if (!fs.existsSync(cliProject)) throw new Error(`ChartForgeX.Markup.Cli project not found at ${cliProject}.`);
...
const bundledCli = assertChildPath(path.join(extensionRoot, 'tools', 'ChartForgeX.Markup.Cli'), extensionRoot);
...
if (options.skipRestore) commonDotnetArgs.push('--no-restore');
...
console.log(`Publishing ChartForgeX.Markup.Cli (${options.configuration}, ${options.framework}, portable fallback)...`);
...
for (const rid of options.runtimeIdentifiers) {
  ...
  console.log(`Publishing ChartForgeX.Markup.Cli (${options.configuration}, ${options.framework}, ${rid}, self-contained)...`);
  run('dotnet', [
    ...commonDotnetArgs,
    '-r', rid,
    '--self-contained', 'true',
...
const outputRoot = assertChildPath(
  path.isAbsolute(options.outputDirectory) ? options.outputDirectory : path.join(extensionRoot, options.outputDirectory),
  extensionRoot
);
...
const packageArgs = [...vsce.prefix, 'package', '--allow-missing-repository', '--out', vsixPath];
if (options.preRelease) packageArgs.push('--pre-release');
...
if (options.publishMarketplace) {
  ...
  if (!token || !token.trim()) throw new Error('VSCE_PAT is required when publishing to the Visual Studio Marketplace.');
  const publishArgs = [...vsce.prefix, 'publish', '--packagePath', vsixPath];
  if (options.preRelease) publishArgs.push('--pre-release');

...
internal static partial class SmokeTests {
    private static void BuildScriptVerifiesReleaseArtifacts() {
        ...
        Assert(qualityWorkflow.Contains("dotnet nuget locals all --clear", StringComparison.Ordinal), "Quality workflow should clear reusable NuGet cache space before package provisioning on Linux runners.");
        Assert(qualityWorkflow.Contains("has_native_aot_compiler", StringComparison.Ordinal), "Quality workflow should name the compiler probe generically because the Linux release gate can use clang or gcc.");
        Assert(qualityWorkflow.Contains("command -v apt-get", StringComparison.Ordinal) && qualityWorkflow.Contains("command -v dnf", StringComparison.Ordinal) && qualityWorkflow.Contains("command -v yum", StringComparison.Ordinal) && qualityWorkflow.Contains("command -v apk", StringComparison.Ordinal), "Quality workflow should support common Linux package managers on Linux runners.");
        ...
        Assert(qualityWorkflow.Contains("apt_packages+=(gcc)", StringComparison.Ordinal) && qualityWorkflow.Contains("apt_packages+=(zlib1g-dev)", StringComparison.Ordinal), "Quality workflow should only install missing Ubuntu Native AOT prerequisites on Linux runners.");

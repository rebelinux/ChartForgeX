...
public sealed partial class PngChartRenderer {
    private static void DrawTreemap(RgbaCanvas c, Chart chart, ChartRect basePlot) {
        ...
        foreach (var tile in tiles) {
            ...
            c.FillRoundedRectVerticalGradient(rect.X, rect.Y, rect.Width, rect.Height, radius, Blend(ChartColor.White, color, 0.88), Blend(ChartColor.Black, color, 0.94));
            c.StrokeRoundedRect(rect.X, rect.Y, rect.Width, rect.Height, radius, ApplyOpacity(chart.Options.Theme.CardBackground, 0.70));
            DrawTreemapTileLabels(c, chart, rect, FormatX(chart, tile.Point.X), FormatValue(chart, tile.Point.Y), color);
        }
    }

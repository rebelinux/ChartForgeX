...
public sealed partial class SvgChartRenderer {
    private static void DrawBubbles(StringBuilder sb, Chart chart, int index, ChartRect plot, ChartMapper map) {
        ...
        for (var pointIndex = 0; pointIndex + 1 < series.Points.Count; pointIndex += 2) {
            ...
            sb.AppendLine($"<circle data-cfx-role=\"bubble\" data-cfx-series=\"{index}\" data-cfx-point=\"{item}\" cx=\"{F(x)}\" cy=\"{F(y)}\" r=\"{F(radius)}\" fill=\"{color.ToCss()}\" fill-opacity=\"0.30\" stroke=\"{color.ToCss()}\" stroke-opacity=\"0.88\" stroke-width=\"2\" role=\"img\" aria-label=\"{Escape(summary)}\"/>");
            sb.AppendLine($"<circle data-cfx-role=\"bubble-highlight\" data-cfx-series=\"{index}\" data-cfx-point=\"{item}\" cx=\"{F(x - radius * 0.28)}\" cy=\"{F(y - radius * 0.28)}\" r=\"{F(Math.Max(1.4, radius * 0.18))}\" fill=\"{chart.Options.Theme.CardBackground.ToCss()}\" opacity=\"0.26\"/>");
            if (ShouldDrawDataLabels(chart, series)) DrawDataLabel(sb, chart, FormatValue(chart, size), x, y - radius - 12, plot);
        }
    }

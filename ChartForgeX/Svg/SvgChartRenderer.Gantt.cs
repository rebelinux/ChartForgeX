...
public sealed partial class SvgChartRenderer {
    private static void DrawGantt(StringBuilder sb, Chart chart, ChartRect basePlot, string id) {
        ...
        for (var i = 0; i < items.Count; i++) {
            ...
            if (chart.Options.ShowGrid) sb.AppendLine($"<line x1=\"{F(plot.Left)}\" y1=\"{F(centerY)}\" x2=\"{F(plot.Right)}\" y2=\"{F(centerY)}\" stroke=\"{t.Grid.ToCss()}\" stroke-width=\"1\" opacity=\"0.22\"/>");
            if (chart.Options.ShowAxes) {
    ...
    private static List<GanttItem> BuildGanttItems(Chart chart) {
        ...
        for (var i = 0; i < chart.Series.Count; i++) {
            ...
            items.Add(new GanttItem(i, series.Name, Math.Min(range.X, range.Y), Math.Max(range.X, range.Y), Clamp(metadata.X, 0, 1), (int)Math.Round(metadata.Y), flags.X >= 0.5, series.Color ?? Color(chart, i)));
        }
    ...
    private static void DrawGanttTask(StringBuilder sb, Chart chart, GanttItem item, string id, int row, double left, double y, double width, double height) {
        ...
        sb.AppendLine($"<rect data-cfx-role=\"gantt-task\" data-cfx-row=\"{row}\" data-cfx-start=\"{F(item.Start)}\" data-cfx-end=\"{F(item.End)}\" data-cfx-progress=\"{F(item.Progress)}\" role=\"img\" aria-label=\"{Escape(summary)}\" x=\"{F(left)}\" y=\"{F(y)}\" width=\"{F(width)}\" height=\"{F(height)}\" rx=\"{F(radius)}\" fill=\"{item.Color.ToCss()}\" opacity=\"0.20\"/>");
        if (progressWidth > 0.5) sb.AppendLine($"<rect data-cfx-role=\"gantt-progress\" x=\"{F(left)}\" y=\"{F(y)}\" width=\"{F(progressWidth)}\" height=\"{F(height)}\" rx=\"{F(radius)}\" fill=\"url(#{id}-seriesFill{item.SeriesIndex})\"/>");
        sb.AppendLine($"<rect data-cfx-role=\"gantt-task-border\" x=\"{F(left + 0.5)}\" y=\"{F(y + 0.5)}\" width=\"{F(Math.Max(0, width - 1))}\" height=\"{F(Math.Max(0, height - 1))}\" rx=\"{F(Math.Max(0, radius - 0.5))}\" fill=\"none\" stroke=\"{t.CardBackground.ToCss()}\" stroke-opacity=\"0.72\"/>");
        var inset = Math.Min(radius, width / 3);
        if (width > inset * 2 + 3) sb.AppendLine($"<line data-cfx-role=\"gantt-task-highlight\" x1=\"{F(left + inset)}\" y1=\"{F(y + 1.2)}\" x2=\"{F(left + width - inset)}\" y2=\"{F(y + 1.2)}\" stroke=\"#fff\" stroke-opacity=\"0.25\" stroke-width=\"1\" stroke-linecap=\"round\"/>");
        if (chart.Options.ShowDataLabels && width >= 74) DrawSvgTextCenteredX(sb, chart, "gantt-progress-label", FormatPercent(item.Progress), left + width / 2, y + height / 2, HeatmapTextColor(item.Color), t.DataLabelFontSize, width - 8, "750", t.CardBackground, 2.2);
    }
    ...
    private static void DrawGanttMilestone(StringBuilder sb, Chart chart, GanttItem item, string id, double x, double centerY, double rowHeight) {
        ...
        sb.AppendLine($"<polygon data-cfx-role=\"gantt-milestone\" data-cfx-start=\"{F(item.Start)}\" role=\"img\" aria-label=\"{Escape(summary)}\" points=\"{points}\" fill=\"url(#{id}-seriesFill{item.SeriesIndex})\" stroke=\"{chart.Options.Theme.CardBackground.ToCss()}\" stroke-opacity=\"0.72\"/>");
    }
    ...
    private static void DrawGanttDependency(StringBuilder sb, ChartRect plot, double fromX, double fromY, double toX, double toY, ChartColor color) {
        ...
        var path = "M " + F(Clamp(fromX + 6, plot.Left, plot.Right)) + " " + F(fromY) + " H " + F(midX) + " V " + F(toY) + " H " + F(endX);
        sb.AppendLine($"<path data-cfx-role=\"gantt-dependency\" d=\"{path}\" fill=\"none\" stroke=\"{color.ToCss()}\" stroke-width=\"1.35\" stroke-linecap=\"round\" stroke-linejoin=\"round\" stroke-dasharray=\"5 5\" opacity=\"0.72\"/>");
        sb.AppendLine($"<path data-cfx-role=\"gantt-dependency-arrow\" d=\"M {F(endX)} {F(toY - 4)} L {F(Clamp(toX, plot.Left, plot.Right))} {F(toY)} L {F(endX)} {F(toY + 4)}\" fill=\"none\" stroke=\"{color.ToCss()}\" stroke-width=\"1.35\" stroke-linecap=\"round\" stroke-linejoin=\"round\" opacity=\"0.72\"/>");
    }
    ...
    private static void DrawGanttToday(StringBuilder sb, Chart chart, ChartRect plot, double min, double max) {
        ...
        sb.AppendLine($"<line data-cfx-role=\"gantt-today\" x1=\"{F(x)}\" y1=\"{F(plot.Top)}\" x2=\"{F(x)}\" y2=\"{F(plot.Bottom)}\" stroke=\"{t.Warning.ToCss()}\" stroke-width=\"1.6\" stroke-dasharray=\"6 5\"/>");
    ...
    private readonly struct GanttItem {
        public GanttItem(int seriesIndex, string name, double start, double end, double progress, int dependsOn, bool milestone, ChartColor color) {

...
public sealed partial class SvgChartRenderer {
    private static void DrawProgressBar(StringBuilder sb, Chart chart, ChartRect plot) {
        ...
        for (var i = 0; i < values.Length; i++) {
            ...
            var label = TrimSvgLabelToWidth(FormatX(chart, point.X), t.TickLabelFontSize, labelWidth - 8);
            var color = ProgressItemColor(series, t, i);
            var ratio = Clamp(point.Y / maximum, 0, 1);
            var filledWidth = barArea * ratio;
            sb.AppendLine($"<text data-cfx-role=\"progress-label\" data-cfx-point=\"{i}\" x=\"{F(plot.Left + labelWidth - 8)}\" y=\"{F(y + t.TickLabelFontSize / 3.0)}\" text-anchor=\"end\" fill=\"{t.MutedText.ToCss()}\" font-family=\"{SvgFontFamily(t.FontFamily)}\" font-size=\"{F(t.TickLabelFontSize)}\" font-weight=\"700\">{Escape(label)}</text>");
            ...
            if (showValues) {
                ...
                sb.AppendLine($"<text data-cfx-role=\"progress-value\" data-cfx-point=\"{i}\" x=\"{F(startX + barArea + 12)}\" y=\"{F(y + t.DataLabelFontSize / 3.0)}\" fill=\"{t.Text.ToCss()}\" font-family=\"{SvgFontFamily(t.FontFamily)}\" font-size=\"{F(t.DataLabelFontSize)}\" font-weight=\"800\">{Escape(value)}</text>");
            }
        }
